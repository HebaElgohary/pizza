  ...
  return (
   ...
   <div className='!space-y-2'>
    <div className=' flex flex-col gap-2 !p-11 '>
        <Label htmlFor='size' className='text-lg font-bold text-center'>pick your size</Label>
        <RadioGroupDemo sizes={item.sizes} item={item} ></RadioGroupDemo>
    </div>
    
     <div className=' !p-11 flex flex-col gap-2'>
            <Label htmlFor='extra' className='text-lg font-bold text-center'>Any Extras?</Label>
...
import {
 ...
 function RadioGroupDemo({sizes,item}:{sizes:Size[],item:productWithPayLoad}) {
  return (
    <RadioGroup defaultValue="comfortable">
        { sizes.map((size)=>
      <div key={size.id} className="flex items-center gap-3">
        <RadioGroupItem value={size.name} id="r1" className='border-primary' />
        <Label htmlFor="size" className='text-gray-500'>{size.name} ({formatCurrency(size.price+item.basePrice)})</Label>
      </div>
 )}
    </RadioGroup>
  )
  ...
  return (
    ...
    <div className="flex flex-col gap-6">
      ...
      <div key={extra.id} className="flex items-center gap-3">
        ...
        <Label htmlFor="extras" className='text-gray-500'>{extra.name } {formatCurrency(extra.price)}</Label>
      </div>
)}
